...
/**
 * Prior on position in a Cartesian frame plus a bias for measurement errors
 * caused by the environment or GPS spoofing.
 * Possibilities include:
 *   ENU: East-North-Up navigation frame at some local origin
 *   NED: North-East-Down navigation frame at some local origin
 *   ECEF: Earth-centered Earth-fixed, origin at Earth's center
 ...
 * @ingroup navigation
 */
class GTSAM_EXPORT BiasedGPSFactor: public NoiseModelFactorN<Pose3, Point3> {
...
public:
   ...
   * @param model Gaussian noise model
   */
  BiasedGPSFactor(Key keyPose, Key keyBias, const Point3& gpsIn, const SharedNoiseModel& model) :
      Base(model, keyPose, keyBias), nT_(gpsIn) {
  }
  ...
  /// vector of errors
  Vector evaluateError(const Pose3& nTb, const Point3& eM,
