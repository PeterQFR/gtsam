...
class MagPoseFactorZ: public NoiseModelFactorN<Pose3> {
private:
  using This = MagPoseFactorZ;
  using Base = NoiseModelFactorN<Pose3>;
  typedef Vector3 Point; //Could be a Vector2 or Vector3 depending on POSE.
  typedef Rot3 Rot;
  ...
  static const int MeasDim = Point::RowsAtCompileTime;
  static const int PoseDim = traits<Pose3>::dimension;
  static const int RotDim = traits<Rot3>::dimension;

  /// Shorthand for a smart pointer to a factor.
  using shared_ptr = std::shared_ptr<MagPoseFactorZ>;

  /// Concept check by type.
  GTSAM_CONCEPT_TESTABLE_TYPE(Pose3)
  GTSAM_CONCEPT_POSE_TYPE(Pose3)
...
public:
   ...
   * the measurement error is expressed in the body frame.
   */
  Vector evaluateError(const Pose3& nPb, OptionalMatrixType H) const override {
    // Predict the measured magnetic field h(x) in the *body* frame.
    // If body_P_sensor was given, bias_ will have been rotated into the body frame.
    Matrix H_rot = Matrix::Zero(MeasDim, RotDim);
    const Point hx = nPb.rotation().unrotate(nM_, H_rot, OptionalNone) + bias_;
    ...
    if (H) {
      ...
      *H = Matrix::Zero(MeasDim, PoseDim);
      const size_t rot_col0 = nPb.rotationInterval().first;
      (*H).block(0, rot_col0+2, MeasDim, RotDim-2) = H_rot.col(2);
    }
